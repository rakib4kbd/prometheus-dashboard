...
function fingerprint(username: string, alertName: string, instance: string): string {
  return createHash('sha256').update(`${username}${alertName}${instance}`).digest('hex');
}
...
async function sendWithRetry(
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      await sendAlertEmail(to, alertName, instance, status);
      return;
    } catch {
      if (i === MAX_RETRIES - 1) throw new Error('Email send failed after retries');
...
async function processAlerts() {
  ...
  for (const alert of alerts) {
    ...
    if (lastSent && now.getTime() - new Date(lastSent).getTime() < EMAIL_THROTTLE) continue;

    try {
      await sendWithRetry(userResult.rows[0].email, alertName, instance, status);
      await pool.query(
